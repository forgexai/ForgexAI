  ...
  try {
    ...
    if (!tokenIdParam) {
      return NextResponse.json(
        { error: "tokenId (mint address or symbol) is required" },
        { status: 400 }
      );
    }
    ...
    if (KNOWN_INVALID_MINTS[tokenId as keyof typeof KNOWN_INVALID_MINTS]) {
      const correctedSymbol =
        KNOWN_INVALID_MINTS[tokenId as keyof typeof KNOWN_INVALID_MINTS];
      console.log(
        `[PRICE] Correcting known invalid mint ${tokenId} to symbol ${correctedSymbol}`
      );
    ...
    if (!MINT_REGEX.test(tokenId)) {
      ...
      if (!testOk) {
        console.log(
          "[PRICE] Mint address not found in Jupiter, trying as symbol:",
          tokenId
        );
        ...
        try {
          ...
          if (tokens.length > 0) {
            ...
            tokenSymbol = tokens[0].symbol;
            console.log(
              "[PRICE] Found token by symbol search:",
              tokenSymbol,
              "->",
              tokenId
            );
    ...
    try {
      ...
      if (mintAccountInfo && mintAccountInfo.data.length >= 82) {
        // Parse supply from mint account data
        // Solana mint account structure:
        // - bytes 0-35: mint authority and supply
        ...
        marketCap = adjustedSupply * parseFloat(usdPrice);

        console.log("[PRICE] Market cap calculated:", {
          supply,
          decimals,
          adjustedSupply,
          marketCap,
      ...
      console.warn("[PRICE] Could not fetch mint info:", mintError);
